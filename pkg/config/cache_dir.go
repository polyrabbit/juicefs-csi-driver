...
func LocalCacheDir(preferGPFS bool) ([]string, bool) { // TODO: support multiple cache dirs?
	hostname, _ := os.Hostname()
	if preferGPFS || strings.HasPrefix(hostname, "gpu-") {
		if path := findGPFSCache(); path != "" { // GPFS can be mounted/umounted dynamically, so probe every time
			...
			return []string{gpfsCache}, true
		}
	}
	return []string{localCacheDir.Load().(string)}, false
}
...
func findGPFSCache() string {
	...
	timer := time.NewTimer(3 * time.Second) // Kubelet only waits 5 seconds(?)
	select {
...
func inRootVolume(dir string) bool {
	return deviceOfPath(dir) == deviceOfPath(hostRoot)
}
